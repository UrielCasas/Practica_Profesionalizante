...
#              <agregar link>
#              datos originales tienen movimiento además de por tipo de transporte
#              por línea 
#              Se contempla los diferentes tipos de transporte
#              "COLECTIVO", "TREN", "SUBTE", "LANCHA" 
#              
...
def importar_dir():
    ...
    #print(df['TIPO_TRANSPORTE'].unique())
    #exit()

    df = df[df['TIPO_TRANSPORTE'] != 'CIRCULAR']

    df = df[df['CANTIDAD'] > 0]
    print(len(df))
    print(df.head())

    df = df.groupby(['DIA_TRANSPORTE', 'TIPO_TRANSPORTE'], sort=False)['CANTIDAD'].sum().reset_index()
    print(len(df))
    print(df.head())

    df.rename(columns={'TIPO_TRANSPORTE': 'Tipo', 'CANTIDAD': 'Cantidad'}, inplace=True)
    ...
    def es_pandemia(fecha):
        ...
        return True if pandemia[0] <= fecha <= pandemia[1] else False
    ...
    df = df[['DiaSemana', 'Tipo', 'Feriado', 'Estacion', 'Clases', 'Pandemia', 'Cantidad']]
...
def procesar_df():
    ...
    data_encoded = pd.get_dummies(df, columns=["Estacion", "Tipo"], drop_first=True)
...
def graficar():
    ...
    tipos = ["COLECTIVO", "TREN", "LANCHA", "SUBTE"]
    ...
    tipos_combo = ttk.Combobox(v, values=tipos, width=25)
    tipos_combo.config(state='readonly')
    tipos_combo.set("Selecciona un tipo transp.")
    tipos_combo.place(y=100, x=115)
    ...
    ttk.Checkbutton(v, text="Clases", variable=chk_clases_var).place(y=140, x=115)
    ttk.Checkbutton(v, text="Feriado", variable=chk_feriado_var).place(y=140, x=185)
    ttk.Checkbutton(v, text="Pandemia", variable=chk_pandemia_var).place(y=140, x=255)
    ...
    def ok():
        ...
        if tipos_combo.get() not in tipos:
            messagebox.showinfo("Alerta","Por favor seleccione un tipo de transporte")
            return
...
def mostrar_grafico(nuevo_punto):
    ...
    nuevo_punto_encoded = pd.get_dummies(nuevo_punto, columns=["Estacion","Tipo"], drop_first=True)
    ...
    df_pred_encoded = pd.get_dummies(df_pred, columns=["Estacion","Tipo"], drop_first=True)
...
ttk.Button(frame_controls, text="Importar directorio", command=importar_dir).pack(fill="x", pady=5)
