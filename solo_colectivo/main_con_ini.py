...
max_iter_ini        = 0       # variable global para guardar valor de max_iter
mostrar_preview_ini = False   # guarda si checkbox de mostrar vista previa está tildado al abrir la aplicación
test_size_ini       = 0       # variable global para guardar valor de test_size
random_state_ini    = 0       # variable global para guardar valor de random_state
...
def entrenar(msj=True):
    global X, y, model, X_train, y_train, X_test, y_test, df,test_size_ini,random_state_ini
    if model is None:
    ...
    # X_train (features) y y_train (variable objetivo): valores para entrenar el modelo
    # X_test y y_test --> probar predicciones del modelo, y comparar predicciones con realidad
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size_ini, random_state=random_state_ini)
...
def verificar():
    ...
    messagebox.showerror("Resultado", f"Exactitud del modelo: {acc:.2f}\nLog Loss: {loss:.4f}")
...
def configuracion():
    global max_iter_ini,test_size_ini,random_state_ini,mostrar_preview_ini,config
    # Crea la ventana para solicitar al usuario datos
    v = tk.Toplevel(root)
    ...
    entry_test_size =  tk.Entry(v)
    entry_test_size.insert(0,str(test_size_ini))
    ...
    entry_random_state.insert(0,str(random_state_ini))
    ...
    entry_max_iter.insert(0,str(max_iter_ini))
    entry_max_iter.place(y=80, x=200)

    
    chk_mostrar_preview_var = tk.BooleanVar(value=mostrar_preview_ini)
    ttk.Checkbutton(v, text="Mostra Previsualización", variable=chk_mostrar_preview_var).place(y=110, x=115)

    def ok():
        global max_iter_ini, mostrar_preview_ini, test_size_ini, random_state_ini, model  
        ...
        config["mostrar_previsualizacion"] = int(chk_mostrar_preview_var.get())
      
        archivo_config = open(f"{app_dir}/config.json",'w')
        json.dump(config, archivo_config)
        archivo_config.close()

        max_iter_ini         = config['max_iter']
        mostrar_preview_ini  = config['mostrar_previsualizacion']
        test_size_ini        = config['test_size']
        random_state_ini     = config['random_state']

        model = LogisticRegression(max_iter=max_iter_ini) # Modelo de regresión logística
...
archivo_config = open(f"{app_dir}/config.json",'r')
...
print(mostrar_preview_ini)
chk_preview_var.set(mostrar_preview_ini)
print(chk_preview_var)
