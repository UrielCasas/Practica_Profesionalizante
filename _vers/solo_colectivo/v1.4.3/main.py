...
def crear_archivo_config():
    global config
    try:
        config = {"mostrar_preview" : True, "random_state" : 2100, "max_iter" : 800, "test_size" : 0.1}
        archivo_config = open(f"{app_dir}/config.json",'w')
        json.dump(config, archivo_config)
        archivo_config.close()
        return True
...
try:
    ...
    if not es_int(config["max_iter"]) or not es_int(config["random_state"]) \
        or not es_float(config["test_size"]):
        if not crear_archivo_config():
            messagebox.showerror("Error Fatal", "Error fatal no se puede ejecutar la aplicación.\nContacte al servicio técnico")
            exit()
        
        messagebox.showinfo("Mensaje", "Se creo un nuevo .json porque se encontro una anomalia en el contenido.")
