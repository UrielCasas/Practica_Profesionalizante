...
def administrar_feriados():
    ...
    def cerrando():
        if hubo_modificacion:
            messagebox.showwarning("Atención", "Para que los cambios tengan efecto debe reiniciar la aplicación")
        v.destroy()

    def marcar_feriado():
        nonlocal hubo_modificacion
    ...
    v.protocol("WM_DELETE_WINDOW", cerrando)

    ttk.Button(v, text="Feriado", command=marcar_feriado).place(y=210, x=80)
    ttk.Button(v, text="No Feriado", command=desmarcar_feriado).place(y=210, x=180)

    ttk.Button(v, text="Cerrar", command=cerrando).place(y=260, x=250)
...
menu_hacer.add_separator()
menu_hacer.add_cascade(label="Feriados", command=administrar_feriados)
